...
if __name__ == "__main__":
    ...
    for algo in ["", "vi_", "f_vi_"]:
        with open(f"{algo}policy_{ms}_{mm}.json") as pfile:
            pi = json.load(pfile)
        with open(f"{algo}value_{ms}_{mm}.json") as vfile:
            V = json.load(vfile)
            V = eval(S, A, probs, gamma, V, pi, 0.000001)

        with open(f"precise_{algo}value_{ms}_{mm}.json", "w") as value_file:
